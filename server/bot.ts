    ...
    while (photoQueue.has(telegramId) && photoQueue.get(telegramId)!.length > 0) {
      ...
      try {
        ...
        if (unrecognizedItems.length > 0) {
            ...
            for (const itemName of unrecognizedItems.slice(0, 5)) { responseText += `• \`${itemName}\`\n`; }
            if (unrecognizedItems.length > 5) { responseText += `...и еще ${unrecognizedItems.length - 5}.\n`; }
            responseText += `\n💡 Вы можете скопировать эти названия, исправить их и отправить мне текстом в формате \`Название|Количество\`, чтобы добавить их вручную.\n\n`;
        }
        ...
        if (itemsToShow.length > 0) {
            ...
            for (const item of itemsToShow) { responseText += `• ${item.name} (${item.quantity})\n`; }
            if (newEnrichedItems.length > 5) { responseText += `... и еще ${newEnrichedItems.length - 5} предметов\n`; }
        ...
        // --- КОНЕЦ ВСЕГО РАБОЧЕГО КОДА ---

      } catch (error) {
        console.error('Ошибка при обработке фото из очереди:', error);
        await ctx.reply('❌ Произошла ошибка при обработке одного из скриншотов. Он был пропущен.');
      }
    }
  } finally {
...
bot.on('photo', async (ctx) => {
  ...
  // Проверяем сессию, ПРЕЖДЕ чем добавлять в очередь
  if (!ctx.session?.sessionId) {
  ...
  // Добавляем фото в личную очередь пользователя
  if (!photoQueue.has(telegramId)) {
  ...
  // Если обработчик "спит", запускаем его
